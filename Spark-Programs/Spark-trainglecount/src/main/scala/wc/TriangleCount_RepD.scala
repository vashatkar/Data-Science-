...
//    try { hdfs.delete(new org.apache.hadoop.fs.Path(args(1)), true) } catch { case _: Throwable => {} }
    ...
    val twitterDS = sparksession.read.format("csv").schema(input_schema)
                    .load(args(0))
                    .where($"user1" < args(2).toLong && $"user2" < args(2).toLong)

    // This will ensure that the spark automatically choose the "Partition and Broadcast" for optimizer
    val broadcastDS = functions.broadcast(twitterDS)

    val PATH2DS : DataFrame =
      twitterDS.as("dataset1").join(broadcastDS.as("dataset2"), $"dataset1.user2" === $"dataset2.user1")
      .select($"dataset1.user1", $"dataset2.user2")

    val TRIANGLEDS: Dataset[(Row, Row)] = PATH2DS.as("table1").joinWith(twitterDS.as("table2"),
      $"table1.user1" === $"table2.user2" && $"table1.user2" === $"table2.user1" )
