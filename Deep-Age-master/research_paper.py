...
def research_paper_model(input_shape, classes = None, dropout_rate = 0.2):
    model = Sequential()
    ...
    model.add(Conv2D(strides=(1,1) ,filters = 256 , kernel_size = (5,5), name= "conv_2"))
    model.add(Activation('relu'))
    ...
    model.add(Conv2D(strides=(1,1) ,filters = 256 , kernel_size = (3,3), name= "conv_3"))
    model.add(Activation('relu'))
    ...
    model.add(Dense(512 , name = 'Fully_connected_4' ))
    model.add(Activation('relu'))
    model.add(Dropout(rate = dropout_rate) )
    
    model.add(Dense(512 , name = 'Fully_connected_5' ))
    model.add(Activation('relu'))
    model.add(Dropout(rate = dropout_rate) )
